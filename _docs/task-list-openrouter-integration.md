...
**Objective**: Integrate OpenRouter into Stagehand Agent QA mode

**Prerequisites**:
- Task SA.3 from task-list-stagehand-agent.md (runStagehandAgentQA implementation)
- Tasks OR.1 and OR.2 from this list
...
In CLI handler (wherever `runStagehandAgentQA` is called), ensure OpenRouter config is loaded:
...
**3.3. Update Lambda Handler (if exists)**

Similar validation in Lambda handler to ensure OpenRouter API key is present.
